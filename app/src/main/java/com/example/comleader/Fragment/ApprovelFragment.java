...
public class ApprovelFragment extends Fragment {
    ...
    ArrayList<MemModel> List, FLis;
    DatabaseReference SearchRef;

    public View onCreateView(@NonNull LayoutInflater inflater,
                             ViewGroup container, Bundle savedInstanceState) {
        View root = inflater.inflate(R.layout.fragment_approvel, container, false);

        S_PhoneS = root.findViewById(R.id.search_view_Apr);

        recyclerView = root.findViewById(R.id.apr_mem_recy);
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    if (dataSnapshot.exists()) {
                        ...
                        for (int y = 0; y < List.size(); y++){
                            ...
                            if(List.get(y).getSuspend().equals("A")){
                        ...
                        if(getActivity() != null){
                            ...
                            adapter = new Folding_Cell_Adapter(getActivity(), FLis);
    ...
    private void searchIt(String newText) {
        ...
        for(MemModel product : List){
            if((product.getName() + " " + product.getNumber() + " " + product.getAddress()).toLowerCase().contains(newText.toLowerCase()) && product.getSuspend().equals("A")){
                Newlist.add(product);
            }
        }
